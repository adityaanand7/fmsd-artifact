...
public class StaticAnalyser extends BodyTransformer {
	...
	public static HashMap<SootMethod, Boolean> methodstoanalyze = new HashMap<>();
	public static HashMap<SootMethod, Boolean> methodstoanalyzecopy = new HashMap<>();
	String[] ignoreFuncs = {
	...
	// Function that returns the summary size
	...
	protected void internalTransform(Body body, String phasename, Map<String, String> options) {
		...
		if(flag) {
			...
			//Iterator<MethodOrMethodContext> edges = cg.sourceMethods();
			SootMethod s = Scene.v().getMainMethod();
			//System.out.println("Method name :" + s.getSignature());
			methodstoanalyze.put(s,false);
			int size = methodstoanalyze.size();
			while(true) {
				for(SootMethod m : methodstoanalyze.keySet()) {
					Iterator<Edge> it = cg.edgesOutOf(m);
					while (it.hasNext()) {
						Edge edge = it.next();
						SootMethod method = edge.tgt();
						methodstoanalyzecopy.put(method,false);
					}
				}
				//System.out.println("Methods to analyze : "+ methodstoanalyze);
				//System.out.println("Methods to analyze copy : "+ methodstoanalyzecopy);
				methodstoanalyze.putAll(methodstoanalyzecopy);
				if(methodstoanalyze.size() == size) {
					break;
				}
				size = methodstoanalyze.size();
				//System.out.println("Size :"+ size);
			}
			//System.out.println("Size of callgraph : "+ methodstoanalyze.size());
		}
		boolean tempfl = false;
		if(methodstoanalyze.containsKey(body.getMethod())) {
			if(methodstoanalyze.get(body.getMethod()).equals(false)){
				tempfl =true;
			}
		}
		if (!tempfl) {
		...
		methodstoanalyze.put(body.getMethod(),true);
		//System.out.println("Method is : " + body.getMethod().getName());
